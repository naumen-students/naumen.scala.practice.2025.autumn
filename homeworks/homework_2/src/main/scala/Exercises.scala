object Exercises {
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def primeFactor(number: Int): Seq[Int] = number match {
        case n if n <= 1 => Seq.empty
        case n => 
            @annotation.tailrec
            def factors(num: Int, div: Int, acc: Set[Int]): Set[Int] = (num, div) match {
                case (x, _) if x <= 1 => acc
                case (x, d) if d * d > x => if (x > 1) acc + x else acc
                case (x, d) if x % d == 0 => factors(x / d, d, acc + d)
                case (x, d) => factors(x, d + 1, acc)
            }
    
        factors(n, 2, Set.empty).toSeq.sorted
}
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def getWeight(radius: Int, density: Double): Double = {
            (4.0 / 3.0) * java.lang.Math.PI * java.lang.Math.pow(radius, 3) * density
        }
        ballsArray.toSeq
          .sortBy{ case ( _, (radius, density)) => getWeight(radius, density) }
