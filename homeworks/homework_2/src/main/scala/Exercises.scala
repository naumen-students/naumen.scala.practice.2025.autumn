object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @annotation.tailrec
    def factors(n: Int, d: Int, acc: List[Int]): List[Int] = {
      if (n <= 1) acc
      else if (n % d == 0) factors(n / d, d, d :: acc)
      else factors(n, d + 1, acc)
    }
    
    factors(number, 2, List()).distinct.reverse
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    def weight(radius: Int, density: Double): Double = 
      (4.0 / 3.0) * density * java.lang.Math.PI * math.pow(radius, 3)

    ballsArray
      .map { case (key, (r, d)) => (key, weight(r, d)) }
      .toSeq
      .sortBy(_._2)
      .map(_._1)
 }
